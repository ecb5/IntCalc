...
},
{
  ...
  "body": " A Quick Review of Differential Calculus  In Differential Calculus: From Practice to Theory we saw that the notation for an infinitesimal difference, , can be thought of as a refinement of the change of symbol which you learned when you were studying straight lines. The distinction is that we think of as a measurable distance between and whereas represents and infinitesimal distance. To be sure we also learned in Differential Calculus: From Practice to Theory that the notion of an infinitesimal is very problematic as a logical foundation of our investigations. But as a way of thinking; as an appeal to intuition, it can be very useful. Naturally, any conclusions we draw from our intuitive approach will have to be confirmed logically but we needn t fuss about that yet.  By thinking in this way we found that we could extend the idea of the slope of a straight line to curved lines. The slope of a straight line is given by (the change of over the change of ) so the slope of a curved line at the point is given by the notation n . It is necessary to specify the slope of the curve at the particular point because it is clear that the slope of a curved line might change from point to point.  This generalization of the notion of slope is very useful in analyzing scientific problems because the interpretation of will vary depending on what physical quantities and happen to represent. To take one simple interpretation if the variable measures a vertical displacement and the variable measures the passage of time then represents the velocity in the vertical direction. (We would probably actually use instead of in this case in order evoke the word time , but the names we choose for our variables are generally irrelevant. We choose a name for a variable simply to help us remember what it represents.) For example, the position of a ball tossed vertically with an initial velocity , from an initial height , will be if is measured in meters and is measured in seconds.  Thus the ball s velocity will be and its acceleration will be which is, of course, the rate of acceleration of an object falling near the surface of the earth, as determined by Galileo   One exercise in Differential Calculus asked you to show that if represents the shape of a catenary curve then satisfies the differential equation      A later exercise had you show that in fact the function satisfies equation . Repeat that computation by computing and and showing that satisfies equation .    A natural question is How did we find ? This is our current topic. In the simplest formulations Differential Calculus is about differences subtraction while Integral Calculus is about sums addition. That is, given a function , when we differentiate it we find the the infinitesimal difference between two infinitely close points. Integration is, loosely speaking, the reverse of this process.   Zero Differentials  For example, if then . It stands to reason that if we add up all of the differentials of (the s) we will reconstruct .  Naturally, things are not quite this simple. Consider Clearly so the sum of would seem to be the same as the sum of . But clearly whereas . This apparent contradiction clearly needs to be resolved.  One way to resolve it is to form the function and notice that since it follows that Dividing by and thinking of and as slopes, we have . This says that the curve has a slope which is constantly equal to zero. A moment’s thought will suggest that this curve must be a horizontal line for if not then somewhere it should have a nonzero slope. This argument can be made rigorous, but we will not fuss over that right now.    Integral Notation   Notation  To a modern eye the integral sign looks like a stylized S, and that is often how it is described. But in the seventeenth century it wasn t stylized at all. When Calculus was invented the letter S typically took one of several forms, depending on where it appeared in a word. One of these forms was elongated to extend above and below the line of text. Leibniz adopted this as the integral sign. So, it is not really stylized. It is archaic.  Leibniz chose the letter S because it is the first letter of Latin word summa , from which we get the English word sum.    The notation is motivated by the fact that integration (putting together, summing) is, in its simplest formulation, the inverse process of differentiation (taking a difference, subtracting). The summation of differentials is called integration and is denoted by . An integral sign consists of two parts: The symbol which indicates summation, and the differential  in this case.  An integral always represents a sum of differentials. For example, if then so that makes sense because the product is a differential. But is nonsense because alone is not a differential.  Second, notice we did not write . This is because it is not (quite) true. There are an infinite number of possible values of . Fortunately, as we mentioned before, these all differ by constants. Thus we can write something like where is an arbitrary constant. This represents all of the possible functions whose derivative is , so with that in mind is referred to as the indefinite integral or anti-derivative of . These names come from the foundational switch from differentials to limits and derivatives, but we still want to emphasize that we integrate differentials, not functions (so the is not optional).   "
},
{
...
},
{
  ...
  "body": " Zero Differentials  For example, if then . It stands to reason that if we add up all of the differentials of (the s) we will reconstruct .  Naturally, things are not quite this simple. Consider Clearly so the sum of would seem to be the same as the sum of . But clearly whereas . This apparent contradiction clearly needs to be resolved.  One way to resolve it is to form the function and notice that since it follows that Dividing by and thinking of and as slopes, we have . This says that the curve has a slope which is constantly equal to zero. A moment’s thought will suggest that this curve must be a horizontal line for if not then somewhere it should have a nonzero slope. This argument can be made rigorous, but we will not fuss over that right now.  "
},
{
  "id": "FTC-2-10-3",
  "level": "2",
  "url": "FTC-2.html#FTC-2-10-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "integration integral sign "
},
{
  "id": "FTC-3",
  "level": "1",
...
},
{
  ...
  "number": "2.14",
  "title": "",
  "body": " Suppose Show that if then   (Don't let this formula frighten you. Recall that .)   Apply equation the function and compare this to the results in Problem # 60 above.   The formula is called the integration by parts formula and as you can see it is simply a rearrangement of the Product Rule formula. In this form it is exactly what we need to compute the previous integral. Specifically, if we let and , then and (don't worry about the arbitrary constant here for the moment. We'll look at it later.). Substituting these into our integration by parts formula, we have   At this point, we should take note of a few things. First, notice that this was not a substitution; we did not end up with an integral with 's or 's in it. These were introduced just to keep track of the integration by parts formula.  Second, notice that we needed to use the entire integrand. That is, the expression is exactly equal to the the integrand, . Nothing was left out.  Second, notice that like a substitution, the integration by parts technique does not compute the integral. Rather, it replaces that integral with, hopefully, an easier integral. This means that as long as the integration by parts technique is applied correctly, it is not wrong; it just might not be helpful. For example, we could have let and . This would give . Substituting these into equation we get Everything we've done is correct, but we have an integral that is worse than before. Practice is required to us this technique as well. Lots of practice.  Lastly, notice that we suppressed the arbitrary constant when we computed . We will see that this will not alter the result.  "
},
{
...
},
{
  ...
  "number": "2.16",
  "title": "",
  "body": "Notice that in Table we had but we had no analog for the integral of . Use integration by parts on the integral with and to compute the integral.  A Word of Advice  Rather than trying to memorize this integral, just remember to use integration by parts.  "
},
{
  "id": "IntegrationMethods-4-26",
  "level": "2",
  "url": "IntegrationMethods-4.html#IntegrationMethods-4-26",
  "type": "Problem",
  "number": "2.17",
  "title": "",
...
{
  "id": "IntegrationMethods-4-29",
  "level": "2",
  "url": "IntegrationMethods-4.html#IntegrationMethods-4-29",
  "type": "Problem",
  "number": "2.18",
  "title": "",
...
{
  "id": "IntegrationMethods-4-31",
  "level": "2",
  "url": "IntegrationMethods-4.html#IntegrationMethods-4-31",
  "type": "Example",
  "number": "2.19",
  "title": "",
...
{
  "id": "IntegrationMethods-7-12",
  "level": "2",
  "url": "IntegrationMethods-7.html#IntegrationMethods-7-12",
  "type": "Figure",
  "number": "2.61",
  "title": "",

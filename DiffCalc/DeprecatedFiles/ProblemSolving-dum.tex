...
\chapter{Introduction: Some  Advice on Problem Solving}
...
\emph{only} the concepts and techniques of Calculus are needed to
...
more. Really.
 ...
 \section{Using Letters Instead of Numbers}
 \label{sec:using-lett-inst}
 \markright{\sc{} Using Letters Instead of Numbers}

  Before we begin, let's talk about the use  of notation in
  mathematics. Specifically, let's talk about the predilection of all
  math teachers to use letters instead of numbers.

  % \paragraph{ \underline{Generality}}
  % \label{sec:generality}
  Suppose your teacher starts a lecture with the statement, ``Suppose
  $a$ and $b$ are two positive numbers. Then $a \lt{} a+b$ and $b \lt{} a+b$.'' A
  common complaint we have heard from students is that we're making
  the problem harder. This complaint is often accompanied with a
  request to replace $a$ and $b$ with numbers, just to make it easier
  to understand. For example, if $a$ and $b$ are numbers why not
  just say that $a$ is $23$ and $b$ is $17$? In that case it is clear
  that $23 \lt{} 40\ (=23+17)$ and $17 \lt{} 40$. Why do we have to make
  it more complicated?

  This seems like a reasonable point until you think about it a bit
  more. Suppose we use the numbers $a=\frac{\sqrt{11}-\pi}{4}$ and
  $b=2.93\times 10^{1000}$ instead? Is it obvious to you that
  $$\frac{\sqrt{11}-\pi}{4}\lt{}\left(\frac{\sqrt{11}-\pi}{4}\right)+\left(2.93\times
    10^{1000}\right)$$ or
  $$2.93\times
  10^{1000}\lt{}\left(\frac{\sqrt{11}-\pi}{4}\right)+\left(2.93\times
    10^{1000}\right)?$$

  Our point here is that the symbol $a$ is no different than the symbol
  $23$.  Or the symbol $\frac{\sqrt{11}-\pi}{4}$.  These are all valid
  symbols representing numbers. So we treat them all the same.
  For example, all of the following are valid statements:
  \begin{align*}
    a+b &= b+a\\
    23+17 &=17+23, \text{ and }\\
    \left(\frac{\sqrt{d11}-\pi}{4}\right) +\left(2.93\times 10^{1000}\right) &= \left(2.93\times 10^{1000}\right) +\left(\frac{\sqrt{11}-\pi}{4}\right).
  \end{align*}
  The difference is this: If $a=23$ and $b=17$ the first statement
  \emph{includes} the second as a specific case. Similarly if
  $a=\frac{(\sqrt{11}-\pi)}{4}$ and $b=2.93\times 10^{1000}$ the first
  statement includes the third.  The first statement is more
  general. It is true no matter what $a$ and $b$ are.

    \begin{embeddedproblem-enumerate}
      \begin{enumerate}[label={\bf{}(\alph*)}]
        % \begin{multicols}{2}
      \item Use the fact that $\sqrt{7}\lt{}\sqrt{9}$ to show that
        $\frac{\sqrt{7}-\pi}{5}\lt{}0$. Do not use a calculator.
      \item Check the validity of the statements. Are they true?\\
\centerline{$\frac{\sqrt{7}-\pi}{5}\lt{}\left(\frac{\sqrt{7}-\pi}{5}\right)+2.93\times 10^{1000}$}\\
\centerline{$2.93\times 10^{1000}\lt{}\left(\frac{\sqrt{7}-\pi}{5}\right)+2.93\times
                                10^{1000}$}\\
        \hint{Using a calculator on $2.93\times 10^{1000}$ would be
          rather pointless. What about the other number?}
      \item Show that it is not always true that if $a$ and $b$ are
        numbers then
        \begin{equation}
          a\le a+b.\label{eq:TriIneq}
        \end{equation}
      \item Find a restriction on the possible values of $a$ and $b$
        that will guarantee that inequality~(\ref{eq:TriIneq})
        is true.
        % \end{multicols}
      \end{enumerate}
  ...
  The goal of ``keeping things simple'' is laudable and we don't want
  ...
  abstraction'' is to \emph{think} $23$ and $17$, but \emph{write
    down} $a$ and $b$.  As long as you don’t try to do anything that
  relies on the particular properties of $23$ and $17$, you should be
  fine. But you wouldn’t want to say something like $a+b$ is an even
  integer (even though $23+17$ is). The sum $23+17$ is an even integer
  because both $23$ and $17$ are odd integers, but this is special to
  these numbers and won't be true for $a$ and $b$ in general.

    \begin{ProficiencyDrill-enumerate}
      \begin{enumerate}[label={\bf{}(\alph*)}]
        % \begin{multicols}{2}
      \item Suppose $a$ and $b$ are both integers.  Find restrictions
        on the possible values of $a$ and $b$ that will
        \emph{guarantee} that $a+b$ is an even, positive integer.
      \item Now suppose $a$ and $b$ are both numbers. Find
        restrictions on the possible values of $a$ and $b$ that will
        \emph{guarantee} that $a+b$ is an even, positive
        integer.\\
  ...
  %    In general, if $a$ and $b$ are numbers then it is
  %   \emph{not} true that $a+b$ is a positive integer.

  %   However if we narrow the range of values we allow for $a$ and $b$
  %   this will be true. 
  % \end{embeddedproblem}
 
    When looking at $23\times17$ most people are inclined to compute
    this to get $391$. If the problem is: Find the value of
    $23\times17$, then sure, doing the arithmetic is the correct thing
    to do. But the last time you were asked to do multiplication for
    its own sake you were probably $11$ years old. Elementary
...
$23\times17$ is divisible by $17$. Do you see that computing
$23\times17=391$ will only make the problem harder?

Your college professors will \emph{never} ask you to do an arithmetic
...
\centerline{$1, 2, 10, 110, 220, 1100, 12100, 24200, 121000, 1331000,
  \ldots$ }
or like this:\\
\centerline{$(2^05^011^0), (2^15^011^0),
  (2^15^111^0), (2^15^111^1),
  (2^25^111^1),
  (2^25^211^1), (2^25^211^2), \ldots$}.
...
\begin{enumerate}[label={\bf{}(\alph*)}]
  \item 
Is the number $2^{24}5^{49}11^{1002}$ a perfect square?  If so, what
  is its square root?
\item 
  What about $2^{24}5^{50}11^{1002}$?  Was your calculator of any use on this?  Notice
  that these numbers were completely factored into their prime
  factorizations.  Did that help? How?
\item Now suppose that $n$ is a positive integer. Is
  $2^{2n}5^{3n}11^{4n}$ always a perfect square? If not, for which
  values of $n$ will it be a perfect square?
...
\end{embeddedproblem}
 ...
 \section{Substitution, or Making Things ``Easy on the Eyes''}
 \label{sec:an-easy-problem-alg}
...
substitution is one way to re-express a complex problem.
  ...
  again.  On the right the coefficient of $x$ is $\pi^3-\sqrt{2}.$
  That's just a number. It is approximately $29.6$ but who cares? For
  purposes of algebraically isolating $x$ all we need to know is that
  it is a number. Rather than calling it $\pi^3-\sqrt{2}$, let's
  just call it something simple, like $A$. Think of it as giving the
  expression $\pi^3-\sqrt{2}$ a nickname; something shorter, and
  easier to remember.
  ...
  But wait. The expression $\sqrt[21]{2^{\frac{11}{2}}}$ is also just a
  number. Let's make it ``easier on the eyes,'' too. If we let
  $B=\sqrt[21]{2^{\frac{11}{2}}}$ we have
  $$
  \frac{\frac{\pi x}{\sqrt{(28-\sqrt[3]{26})^{\frac{1}{3}}}}}{\pi^2-\sqrt{3}}
  - B= Ax.
  $$
  ...
  going. If we let
  $C=\frac{\frac{\pi}{\sqrt{(28-\sqrt[3]{26})^{\frac{1}{3}}}}}{\pi^2-\sqrt{3}}$
  then our equation is
  $$
  Cx-B=Ax
  $$
  which is \emph{much} ``easier on the eyes'' than the original
  equation was. In fact, we can now see that this is really a very
  simple equation and the solution is $x=\frac{B}{C-A}$. Of course we
  inserted $A$, $B$, and $C$ into the problem so now we need to unwind
  our
...
\begin{myexample}
  ...
  Given
  \begin{equation}
    x^{\frac{2}{3}}-4x^{\frac{1}{3}}+3=0\label{eq:QuadInForm1}
  \end{equation}
  solve for $x$.
  ...
  form. Because $x^{\frac{2}{3}}=\left(x^{\frac{1}{3}}\right)^2$ we
  can rewrite equation~(\ref{eq:QuadInForm1}) as
  \begin{equation}
    \left(x^{\frac{1}{3}}\right)^2-4x^{\frac{1}{3}}+3=0\label{eq:QuadInForm2}
  \end{equation}
  and if you look closely you'll see that
  equation~(\ref{eq:QuadInForm2}) is ``quadratic in form.'' That is,
  it is really a quadratic equation -- which we know how to solve --
  in disguise. However it is still rather difficult to see the
  quadratic nature of equation~\ref{eq:QuadInForm2}. To make this
  \emph{very} clear we let
  $$
  z=x^{\frac{1}{3}}.
  $$
  That is, just as in Example~\ref{example:Sub1} we give the
  expression $x^{\frac{1}{3}}$ the nickname, $z$, which is shorter, easier to
  remember, and ``easier on the eyes.'' Then
  equation~(\ref{eq:QuadInForm2}) becomes the quadratic equation:
  $$
  z^2-4z+3=0.
  $$
  ...
  0=z^2-4z+3=(z-1)(z-3)
  $$
  so we have $z=1$ and $z=3$. But, of course, these are not solutions of our
  problem. We have found $z$ (which we inserted into the problem)
  not $x$ (which is what we need to find). To complete the solution we
  need to unwind the substitution we made and solve the two equations
  $$x^{\frac{1}{3}}=1 \text{ and } x^{\frac{1}{3}}=3,$$
  yielding the solutions
    $x=1$ and $x=27$, respectively.
\end{myexample}
...
Examples~\ref{example:Sub1} and~\ref{example:Sub2} it was
straightforward\aside{``Straightforward'' is not the same as
  ``easy.''}  to see how to do that.  There were expressions that were
...
\begin{embeddedproblem}
  ...
  substitution for equation~(\ref{eq:QuadEq}).\\
  \hint{The goal is to eliminate the linear term,
    $bx$. You can see how this would help. If $bx$ were not present in
    equation~\ref{eq:QuadEq} we could easily
    solve $ax^2+c=0.$}
\end{embeddedproblem}


\begin{embeddedproblem}
  Were you able to solve Problem~\ref{problem:QuadSub1}?
  It's actually very hard to do without some guidance. Here's another
  hint. Try the substitution $x=y-\frac{b}{ka}$ where $k$ is a
  parameter to be determined.  That is, make this substitution and
  find the value of $k$ that eliminates the linear term.

  After making this substitution the problem will be given in terms of
  $y$, not $x$. That's OK. Solve the problem in this form, and then
  unwind your substitution back to the original $x$ variable.
\end{embeddedproblem}

  Notice that after making our substitution we
  get:
    $$ a\left(y-\frac{b}{k a}\right)^2+b\left(y-\frac{b}{k
        a}\right)+c=0.  $$ which is \emph{more} complicated, not
  ...
  Once you've found $k$ make the substitution and eliminate the linear
...
%   Example~\ref{example:Sub2} to eliminate the
%   quadratic term from equation~\ref{eq:Cubic}.
% \begin{embeddedproblem-enumerate}[Find the pattern]
%   \vskip-.3in{}
%   \begin{enumerate}[label={\bf{}(\alph*)}]
%   \item Find the substitution  needed to eliminate the quadratic term
%     ($bx^2$) in equation~\ref{eq:Cubic}.
%   \item Now look back at the substitutions used to eliminate the
%     linear term ($x$ to the first power) from the general quadratic,
%     and to eliminate the quadratic term ($x$ to the second power) from
%     the general cubic. Find the pattern and guess what substitution
%     will eliminate the cubic term ($x$ to the third power) from the
...
% would \emph{just use numbers} instead of all of these letters
% everything would be easier.

% For example, if $a$, $b$, and $c$ are the sides of a right triangle
% and $c$ is the hypotenuse then we know from the 
% Given two numbers $a$ and $b$, is it true that $0\lt{} a \lt{} a+b$. A common
%   error at this point is to use specific numbers, like $5$, $7$, to
...
%   call our numbers $a$ and $b$, but that just makes it harder to
%   handle them if we have to combine them in a formula. If $a=7$ and
%   $b=5$ then $7^2+5^2=49+25=99$ which is \emph{much} simpler than
%   $a^2+b^2$. Why make things more complicated by using letters?''

%   Why indeed. This is an entirely valid point. But here's the
%   problem. Nature rarely gives us simple integers like $5$ and $7$ to
%   work with. Suppose instead of $5$ and $7$ we had
%   $a=\frac{\sqrt{7}+\pi}{4}$ and $b=2.93\times 10^3$? Then
%   $a^2+b^2$ becomes $\left(\frac{\sqrt{7}+\pi}{4}\right)^2+ \left(2.93\times
%       10^3\right)^2$. Would you really rather work with that? Is it
...
\section{An ``Easy'' Problem From Geometry}
\label{sec:an-easy-problem-geo}
...
Before reading
further do your best to solve this problem in Example~\ref{example:MaxTriangle}. We'll wait.
\begin{center}
  \begin{minipage}{.8\linewidth}
    \vfill{} {\large\bf{} No, really. Give this problem a serious shot
      \emph{before} you go on. It is not important that you
...
At first it is difficult to see where to begin the
problem\aside{That's why it's called a problem.}.  Don't let this stop
you! In our experience students often give up too soon.

Don't. Do. That.  Keep thinking.

\begin{wrapfigure}[]{r}{1.5in}
  \captionsetup{labelformat=empty}
  \centerline{\includegraphics*[height=1in,width=1.5in]{../Figures/Advice1}}
  \label{fig:Advice1}
\end{wrapfigure}
Since we know the lengths of sides $a$ and $b$ of our triangle let's
draw it. The sketch at the right would be typical. The question is,
what length for side $c$ makes the total area enclosed by the triangle
as large as it can possibly be?

Now what?

Well, this looks like a right triangle doesn't it? If it is a right
triangle, then we can find the length of $c$ via the Pythagorean
Theorem:
$$
c=\sqrt{a^2+b^2},
$$
...
be careful. It \emph{is} a mistake to rely too heavily on the diagrams
...
\lipsum[3]
\begin{Digression-pmt}[Making Mistakes]
...
\mycenterquote{Experience is the name everyone gives to their
  ...
  \emph{after} you recognize it as a mistake.  Everyone makes mistakes
  in the course of solving a problem. The process of making mistakes,
  recognizing them as mistakes, and figuring out why they are mistakes
  is called learning.  The very smartest people, for example
  ...
  \href{https://mathshistory.st-andrews.ac.uk/Biographies/Fermat/}{Pierre
    Fermat},
  \href{https://www.smithsonianmag.com/history/madame-curies-passion-74183598/}{Marie
    Curie},
  \href{https://mathshistory.st-andrews.ac.uk/Biographies/Noether_Emmy/}{Emmy
    N\"other},
  \href{https://mathshistory.st-andrews.ac.uk/Biographies/Einstein/}{Albert
    Einstein}, or
  \href{https://mathshistory.st-andrews.ac.uk/Biographies/Feynman/}{Richard
    Feynman}, made lots of mistakes.  Making mistakes is how they got
  to be smart.
  ...
  because, who cares about those? Making mistakes doesn't typically
  get the attention it deserves.

  An expert is someone who has made every possible mistake. This is
  why your teacher, an expert, will seldom err (and will be
    embarrassed when (s)he does). Each mistake you make reflects the
  level of your current understanding of the problem. Each mistake you
  make takes you a little closer to expertise.  Embrace your mistakes
  and make lots of them!  They are proof that you are making progress.
  But make no mistake (Gasp!) about it, making and embracing a mistake
  is just the first step. You also have to figure out what went wrong.
  ...
  a mistake. Don't do that. It is pointless and
  counter-productive. All it will do is destroy your self
  confidence. Don't do it% \aside{Did we mention that you shouldn't
    % berate yourself when you make a mistake?  You really
    % shouldn't.}
  . Learning Calculus can be hard, but you would not have
  made it this far if you couldn't do it. So keep making mistakes. Ask
  for help when you need it. And don't give up.  You have not failed
...
\begin{wrapfigure}[5]{l}{1.4in}
  \captionsetup{labelformat=empty}
  \centerline{\includegraphics*[height=.7in,width=1.4in]{../Figures/Advice3}}
  \label{fig:Advice3}
\end{wrapfigure}
But from the diagrams we've drawn so far we can see that one end of
side $a$ must be pinned to one end of side $b$ and for each angle
between $a$ and $b$ we have a different possible triangle.  We don't
have to think of both of them as in motion. We can think of one of
them, say $b$, as fixed while the other swings around their common
...
work, we were led to this insight: We can think of $b$ as static, and
we can think of $a$ as swinging freely while pinned to the end of $b.$
...
\begin{wrapfigure}[7]{r}{3in}
  ...
  \label{fig:Advice4}
\end{wrapfigure}
Clearly we need to find the angle between $a$ and $b$ --- call it
$\phi$ as in the sketch at the right --- that maximizes the area of
...
Since we are thinking of $b$ as fixed, we may as well use it as the
base.This makes the height equal to $a\sin(\phi).$ Also, since we're
trying to maximize the area we should probably write down the area
formula: $A=\frac12\text{(b)ase}\times\text{(h)eight}.$ Thus the area
of the triangle is $A=\frac{1}{2}ab\sin(\phi).$

Is it clear that our initial guess was correct? The angle, $\phi,$ that
maximizes the area will be the one whose sine is as large as
possible. That would be $\phi=90^\circ$ so $c=\sqrt{a^2+b^2}$
provides the maximum area.

\begin{embeddedproblem}
  You might be a little uncomfortable with our argument that
  $a\sin(\phi)$ is maximized when $\phi=90^\circ$. If so, then for
  you, this problem is not finished. Possibly we've made a
  mistake. What if we follow our own advice and see why it is a
  mistake, if it is.

Let's replace $a\sin(\phi)$ with $h$ in the
diagram above. Use the Pythagorean Theorem to reason that if
$\phi\lt{}90^\circ$ then $h\lt{}a$. How does this apply to the 
problem of maximizing the area of the triangle?
\vskip-.2in{}
\end{embeddedproblem}
% Can you account for either of these objections? If not, then the
% problem remains unsolved.

% As we said above, $\phi=\frac{\pi}{2}$ is correct so $c=\sqrt{a^2+b^2}$
...
% conjecture -- a guess.
\vskip-2mm{}
Until we can demonstrate that $c=\sqrt{a^2+b^2}$ with a convincing and
...
% \pagebreak[4]
\newpage{}
\section{Our advice, a synopsis:}
\label{sec:our-advice-synopsis}
...
\item[Have an idea.]\ \\  Solving a mathematical problem is a bit like
  ...
  being told to find your way out\aside{Except, of course, that you
    ...
    mind.}. First you have to find a path. Once a path is found, you
...
\item[Really, have an idea. If you don't have a good idea, then
  ...
  problem in Example~\ref{example:MaxTriangle} problem was draw the
  lines $a$ and $b.$ Look back up at  our partial solution and
  see. The first thing we did was make the drawing below:\\
  \centerline{\includegraphics*[height=1in,width=1.8in]{../Figures/Advice1}}
...
\item[Ask for help when you get stuck, not before.] Sometimes ideas
  ...
  fun with my friends\aside{To be clear, your teacher knows that
    ...
    Teachers \alert{want} to help serious students, but non-serious
    students are a waste of time. Be a serious student. Just as 
    importantly, \emph{act} like a serious student.}.'' This is not
...
that worked, and which ideas you had that didn't work. Think about
\alert{why} they did, or didn't work. Do this immediately, while the
problem is still in your mind. Later, if something reminds you of this
problem, think about it some more to see why you were reminded of it.
...
%  \vskip-.3in{}
  Out of all parallelograms with sides $a$ and $b$, which would have the
  largest area?
%  \vskip-.2in{}
...
That is, there is a tendency to add assumptions to familiar looking
problems. Notice how easy it was to believe that our triangle must be
a right triangle just because our first drawing looked like a right
triangle.
...
% \begin{wrapfigure}[]{r}{1.5in}
% \captionsetup{labelformat=empty}
% \centerline{\includegraphics*[height=1.5in,width=1.5in]{../Figures/ParabolicSection1}}
...
\vskip-3mm{}
  ...
  \item Given the following parabola with axis $\overline{AM}$, $\overline{M}$ is the midpoint
    of $\overline{BC}$.\\
    \centerline{\includegraphics*[height=2in,width=2in]{../Figures/ParabolicSection1}}
  \item In part d, what if we insist that $\overline{BC}$ is perpendicular to $\overline{AM}$?
  \end{enumerate}
